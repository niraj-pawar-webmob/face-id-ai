...
# This code is in regards with optimization -- version 1
# import os
...
# This code is in regards with optimization -- version 2
import os
import threading
from typing import List, Optional
import numpy as np
...
import mediapipe as mp
from insightface.app import FaceAnalysis
from .config import DET_SIZE, MAX_FACES_PER_FRAME, FACE_PACK, FACEMESH_REFINE
...
class FaceEngine:
    def __init__(self):
        ...
        # Persistent MediaPipe graphs (much faster than constructing per call)
        self.mp_face_mesh = mp.solutions.face_mesh

        # For whole-image runs (if you ever use it)
        self.mesh = self.mp_face_mesh.FaceMesh(
            static_image_mode=False,
            max_num_faces=MAX_FACES_PER_FRAME,
        ...
        # For per-bbox crops (max 1 face per crop)
        self.mesh_single = self.mp_face_mesh.FaceMesh(
        ...
        # FaceMesh is not guaranteed thread-safe; guard process() if you use multiple workers
    ...
    def landmarks_for_bboxes(self, img_bgr: np.ndarray, bboxes: List[List[int]], expand: float = 0.25) -> List[Optional[np.ndarray]]:
        """
        Run FaceMesh per detection crop (max 1 face per crop) and map landmarks to full-image coords.
        Uses a persistent mesh to avoid per-request graph construction overhead.
        """
        ih, iw = img_bgr.shape[:2]
        out: List[Optional[np.ndarray]] = []
        for (x1, y1, x2, y2) in bboxes:
            ...
            rgb = cv2.cvtColor(crop, cv2.COLOR_BGR2RGB)
            with self._lm_lock:
                res = self.mesh_single.process(rgb)
            if not res.multi_face_landmarks:
                out.append(None); continue
