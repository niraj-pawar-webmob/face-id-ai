...
from ..local_cache import recent_match_cache  # <-- NEW
...
async def enroll_person(
    ...
    try:
        ...
        if return_image:
            resp["bbox"] = bbox
            resp["debug_image_base64"] = render_match_debug_image(
                img, [], [{"bbox": bbox, "score": 1.0}]
            )
        return resp
...
async def match_faces(
    ...
    try:
        ...
        # 2) Match using cache first, then DB (single session)
        ...
        with SessionLocal() as s:
            ...
            for i, det in enumerate(dets):
                ...
                # 2a) Try local cache fast path
                pid_cache, meta_cache, score_cache = recent_match_cache.best_match(
                    emb, th
                )
                if pid_cache:
                    matches_meta.append(
                        (i, pid_cache, meta_cache or {}, float(score_cache))
                    )
                    # If you want to debug:
                    print(f"[cache] hit for person_id={pid_cache}, score={score_cache:.3f}")
                    continue

                # 2b) Cache miss -> fall back to DB
                pid, meta, score = best_match_in_session(emb, th, s)
                if pid:
                    meta = meta or {}
                    matches_meta.append((i, pid, meta, float(score)))
                    # Store this person in cache for future calls
                    recent_match_cache.put(pid, emb, meta)
                    print(f"[db]    MATCH person_id={pid}, score={score:.3f}  (cached now)")
                else:
                    unmatched.append({"bbox": det["bbox"], "score": float(score)})
                    print(f"[db]    NO MATCH  best_score={score:.3f}")
        ...
        if contours_needed and matches_meta:
            ...
            pts_list = engine_faces.landmarks_for_bboxes(
                img_proc, matched_bboxes, expand=0.25
            )
        else:
        ...
        if return_image:
            response["debug_image_base64"] = render_match_debug_image(
                img, matches_out, unmatched
            )
        return response
